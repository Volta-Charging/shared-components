...
class SiteMarker extends Component {
  ...
  render() {
    const { valueL3, valueL2, text, onClick, issues } = this.props
    ...
    const isLarge = (valueL2  !== null) && (valueL3 !== null)

    if (!isLarge) {
      return (<Container markerWidth={width} markerHeight={height} onClick={onClick}>
        <MarkerContainer  markerWidth={width} markerHeight={height}>
          <PieContainer>
            <svg onClick={onClick} width={width} height={height} viewBox={viewBox}>
              <path d={pin(isLarge)} fill={COLOR.BLACK} stroke={COLOR.WHITE} />
            </svg>
          </PieContainer>
          ...
          <PieContainer height={height} width={width} marginBottom={12} padding={4}>
            <PieChart
              fill={valueL3 ? COLOR.BLUE_MEDIUM : COLOR.BLUE_LIGHT }
              height={height - 12}
              width={width - 12}
              value={valueL3 || valueL2}
              marginBottom={10}
            />
          </PieContainer>
          <PieContainer height={height} width={width} marginBottom={12} padding={4}>
